...
class MermaidRenderer {
    ...
    async render(diagram, options = {}) {
        ...
        try {
            ...
            throw new types_1.ConversionError(`Failed to render Mermaid diagram: ${error instanceof Error ? error.message : String(error)}`, 'MERMAID_RENDER_FAILED', { diagramId: diagram.id, originalError: error });
        }
    }
    async validate(code, type) {
        ...
        const hasValidType = diagramTypes.some(type => code.trim().toLowerCase().startsWith(type.toLowerCase()));
        if (!hasValidType) {
     ...
     */
    async renderToBuffer(code, options) {
     ...
     */
    createMermaidHTML(code, options) {
