...
export class MarkdownParser {
   ...
   */
  async parse(content: string, metadata?: ConversionInput['metadata']): Promise<ParsedContent> {
  ...
  private extractMetadata(
    ...
    content: string,
    inputMetadata?: ConversionInput['metadata']
  ): DocumentMetadata {
    ...
    const firstHeading = tokens.find(token => token.type === 'heading' && token.depth === 1) as Tokens.Heading;
   ...
   */
  private processTokens(tokens: Tokens.Generic[]): ContentSection[] {
   ...
   */
  private tokenToSection(token: Tokens.Generic): ContentSection | null {
    switch (token.type) {
      case 'heading':
   ...
   */
  private extractDiagramTitle(code: string): string | undefined {
   ...
   */
  private processListItems(items: Tokens.ListItem[]): string {
   ...
   */
  private processTable(table: Tokens.Table): string {
   ...
   */
  private parseFrontmatter(frontmatter: string): Partial<DocumentMetadata> {
